...
- (instancetype)init {
    ...
    return [self initWithNibName:@"AlivcUserInfoViewController" bundle:bundle];
}
...
- (void)viewDidLoad {
    ...
    UIImage *image = [AlivcImage imageInSmartVideoNamed:[NSString stringWithFormat:@"user_pic_0%d",arc4random() % 4 + 1]];
...
- (void)loadUserData{
    ...
    if (self.type == AlivcUserVCTypeQuVideo) {
        ...
        [self.avatorImageView sd_setImageWithURL:[NSURL URLWithString:[AlivcProfile shareInstance].avatarUrlString] placeholderImage:[AlivcImage imageInSmartVideoNamed:@"test_avator_boy"]];
...
- (void)loadSDKInfo{
    ...
    self.avatorImageView.image = [AlivcImage imageInSmartVideoNamed:@"alivcIcon"];
}
